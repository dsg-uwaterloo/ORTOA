...
inline int getBit(unsigned char c, int bitNum) { return (c >> bitNum) & 1; }
...
bool createEntryParallel(int part, char *paddedVal, unsigned char *masterKey,
                         unsigned char *label) {
  int partSize =
      (VALUE_SIZE / PROXY_NUM_THREADS) + (VALUE_SIZE % PROXY_NUM_THREADS != 0);
  ...
  for (int i = start; i < limit; i++) {
    c = paddedVal[i];
    for (int j = 0; j < 4; j++) {
      crypto_kdf_derive_from_key(tmpAux, crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j), CONTEXT, masterKey);
      label[0] = (char)((tmpAux[0] & 3) ^ getBits(c, 2 * j));
      label += 1;
      crypto_kdf_derive_from_key(label, crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j) + 1 + getBits(c, 2 * j),
                                 CONTEXT, masterKey);
...
Entry constructCreateEntry(std::string &key, std::string &value) {
  ...
  entry.encryptedLabelsA.resize(VALUE_SIZE * 4 *
                                (1 + crypto_secretbox_KEYBYTES));
  ...
  for (int i = 0; i < PROXY_NUM_THREADS; i++) {
    createThreads[i] =
        pool->submit(createEntryParallel, i, &paddedVal[0], masterKey, label);
  }
...
bool getEntryParallel(int part, std::string *key,
                      unsigned char *encryptedLabelA,
                      unsigned char *encryptedLabelB,
                      unsigned char *encryptedLabelC,
                      unsigned char *encryptedLabelD,
  ...
  for (int i = start; i < limit; i++) {
    for (int j = 0; j < 4; j++) {
      crypto_kdf_derive_from_key(tmpAux, crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j), CONTEXT,
                                 (unsigned char *)&masterKeys[*key][0]);

      char oldBits = tmpAux[0] & 3;

      crypto_kdf_derive_from_key(tmpAux, crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j), CONTEXT, newMasterKey);
      ...
      crypto_kdf_derive_from_key(newLabels[0] + 1, crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j) + 1, CONTEXT, newMasterKey);
      crypto_kdf_derive_from_key(newLabels[1] + 1, crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j) + 2, CONTEXT, newMasterKey);
      crypto_kdf_derive_from_key(newLabels[2] + 1, crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j) + 3, CONTEXT, newMasterKey);
      crypto_kdf_derive_from_key(newLabels[3] + 1, crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j) + 4, CONTEXT, newMasterKey);

      crypto_kdf_derive_from_key(oldLabels[0], crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j) + 1, CONTEXT,
                                 (unsigned char *)&masterKeys[*key][0]);
      crypto_kdf_derive_from_key(oldLabels[1], crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j) + 2, CONTEXT,
                                 (unsigned char *)&masterKeys[*key][0]);
      crypto_kdf_derive_from_key(oldLabels[2], crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j) + 3, CONTEXT,
                                 (unsigned char *)&masterKeys[*key][0]);
      crypto_kdf_derive_from_key(oldLabels[3], crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j) + 4, CONTEXT,
                                 (unsigned char *)&masterKeys[*key][0]);
      ...
      crypto_secretbox_easy(encryptedLabelA + crypto_secretbox_NONCEBYTES,
                            newLabels[0 ^ oldBits],
                            crypto_secretbox_KEYBYTES + 1, encryptedLabelA,
                            oldLabels[0 ^ oldBits]);
      crypto_secretbox_easy(encryptedLabelB + crypto_secretbox_NONCEBYTES,
                            newLabels[1 ^ oldBits],
                            crypto_secretbox_KEYBYTES + 1, encryptedLabelB,
                            oldLabels[1 ^ oldBits]);
      crypto_secretbox_easy(encryptedLabelC + crypto_secretbox_NONCEBYTES,
                            newLabels[2 ^ oldBits],
                            crypto_secretbox_KEYBYTES + 1, encryptedLabelC,
                            oldLabels[2 ^ oldBits]);
      crypto_secretbox_easy(encryptedLabelD + crypto_secretbox_NONCEBYTES,
                            newLabels[3 ^ oldBits],
                            crypto_secretbox_KEYBYTES + 1, encryptedLabelD,
                            oldLabels[3 ^ oldBits]);
...
Entry constructGetEntry(std::string &key) {
  ...
  for (int i = 0; i < PROXY_NUM_THREADS; i++) {
    getThreads[i] = pool->submit(getEntryParallel, i, &key, encryptedLabelA,
                                 encryptedLabelB, encryptedLabelC,
                                 encryptedLabelD, newMasterKey);
  }
...
std::string readValueFromLabels(std::string key, std::string labels) {
  ...
  for (int i = 0; i < VALUE_SIZE; i++) {
    for (int j = 0; j < 4; j++) {
      ...
      crypto_kdf_derive_from_key(label00, crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j) + 1, CONTEXT,
                                 (unsigned char *)masterKeys[key].data());
      crypto_kdf_derive_from_key(label01, crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j) + 2, CONTEXT,
                                 (unsigned char *)masterKeys[key].data());
      crypto_kdf_derive_from_key(label10, crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j) + 3, CONTEXT,
                                 (unsigned char *)masterKeys[key].data());
      crypto_kdf_derive_from_key(label11, crypto_secretbox_KEYBYTES,
                                 5 * (4 * i + j) + 4, CONTEXT,
                                 (unsigned char *)masterKeys[key].data());

      c = modifyBits(
          c, 2 * j,
          !memcmp(currLabel, label00, crypto_secretbox_KEYBYTES)   ? 0
          : !memcmp(currLabel, label01, crypto_secretbox_KEYBYTES) ? 1
          : !memcmp(currLabel, label10, crypto_secretbox_KEYBYTES) ? 2
                                                                   : 3);
...
bool putEntryParallel(int part, std::string *key, std::string *paddedVal,
                      unsigned char *encryptedLabelA,
                      unsigned char *encryptedLabelB,
                      unsigned char *encryptedLabelC,
                      unsigned char *encryptedLabelD,
  ...
  for (int i = start; i < limit; i++) {
    ...
    for (int j = 0; j < 4; j++) {
      ...
      crypto_secretbox_easy(encryptedLabelA + crypto_secretbox_NONCEBYTES,
                            newLabel, crypto_secretbox_KEYBYTES + 1,
                            encryptedLabelA, oldLabels[0 ^ oldBits]);
      crypto_secretbox_easy(encryptedLabelB + crypto_secretbox_NONCEBYTES,
                            newLabel, crypto_secretbox_KEYBYTES + 1,
                            encryptedLabelB, oldLabels[1 ^ oldBits]);
      crypto_secretbox_easy(encryptedLabelC + crypto_secretbox_NONCEBYTES,
                            newLabel, crypto_secretbox_KEYBYTES + 1,
                            encryptedLabelC, oldLabels[2 ^ oldBits]);
      crypto_secretbox_easy(encryptedLabelD + crypto_secretbox_NONCEBYTES,
                            newLabel, crypto_secretbox_KEYBYTES + 1,
                            encryptedLabelD, oldLabels[3 ^ oldBits]);
...
Entry constructPutEntry(std::string &key, std::string &value) {
  ...
  for (int i = 0; i < PROXY_NUM_THREADS; i++) {
    putThreads[i] = pool->submit(
        putEntryParallel, i, &key, &paddedVal, encryptedLabelA, encryptedLabelB,
        encryptedLabelC, encryptedLabelD, newMasterKey);
  }
